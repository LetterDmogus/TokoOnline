...
(function (global, factory) {
  ...
  */
  ...
  const SELECTOR_product = '.dropdown-product';
  ...
  class Dropdown extends BaseComponent {
    ...
    show() {
      if (isDisabled(this._element) || this._product.classList.contains(CLASS_NAME_SHOW$7)) {
        return;
      }
      ...
      this._product.classList.toggle(CLASS_NAME_SHOW$7);
    ...
    _completeHide(relatedTarget) {
      ...
      this._product.classList.remove(CLASS_NAME_SHOW$7);
    ...
    _getproductElement() {
      return SelectorEngine.next(this._element, SELECTOR_product)[0];
    }
    ...
    static clearproducts(event) {
      ...
      for (let i = 0, len = toggles.length; i < len; i++) {
        ...
        if (event) {
          ...
          if (context._product.contains(event.target) && (event.type === 'keyup' && event.key === TAB_KEY || /input|select|option|textarea|form/i.test(event.target.tagName))) {
            continue;
          }
    ...
    static dataApiKeydownHandler(event) {
      ...
      if (/input|textarea/i.test(event.target.tagName) ? event.key === SPACE_KEY || event.key !== ESCAPE_KEY$2 && (event.key !== ARROW_DOWN_KEY && event.key !== ARROW_UP_KEY || event.target.closest(SELECTOR_product)) : !REGEXP_KEYDOWN.test(event.key)) {
        return;
      }
      ...
      if (event.key === ARROW_UP_KEY || event.key === ARROW_DOWN_KEY) {
        ...
        Dropdown.getInstance(getToggleButton())._selectproductItem(event);
  ...
  EventHandler.on(document, EVENT_KEYDOWN_DATA_API, SELECTOR_product, Dropdown.dataApiKeydownHandler);
  ...
  class Tab extends BaseComponent {
    ...
    _transitionComplete(element, active, callback) {
      ...
      if (parent && parent.classList.contains(CLASS_NAME_DROPDOWN_product)) {
